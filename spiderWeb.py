...
class upgrader():
	def __init__(self, parsers, on_close=None):
		self.parsers = parsers
		self.on_close = on_close

	def upgrade(self, client, headers, data, on_close=None):
		...
		c = ws_client(client.parent, client.socket, client.info['addr'], data, self.parsers, on_close=on_close)
		c.state = 'WEBSOCK'
...
class ws_client():
	def __init__(self, parent, sock, addr, data=b'', parsers={}, on_close=None):
		self.sock = sock
		self.addr = addr
		self.parent = parent
		self.state = 'HTTP'
	...
	def close(self):
		...
		print('WS got a on_close:', self.on_close)
		if self.on_close:
			self.on_close(self)
